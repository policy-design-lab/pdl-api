...
def create_table_from_json(tablename, injson_path):
    ...
    # Write data into the table in sqllite database
    # todo this is still not complete. This has to be revised later when it is needed
    df.to_sql(tablename, engine)
...
def create_table_from_json_list(tablename, injson_list):
    ...
    # Write data into the table in sqllite database
    # todo this is still not complete. This has to be revised later when it is needed
    df_total.to_sql(tablename, engine)
...
def create_state_table():
    ...
    with engine.connect() as conn:
        for index, row in df.iterrows():
            ...
            insert_command = insert(states).values(state_fips=val_code, state_code=val_name)
...
def create_state_code_table():
    ...
    with engine.connect() as conn:
        for index, row in df.iterrows():
            ...
            print("Insert " + str(val_code) + " " + val_name)
            insert_command = insert(statecodes).values(state_code=val_code, state_name=val_name)
...
def create_practice_table():
    ...
    with engine.connect() as conn:
        for i, row in df.iterrows():
            ...
            insert_command = insert(practices).values(practice_code=val_code, practice_name=val_name)
...
def create_summary_table():
    ...
    with engine.connect() as conn:
        for index, row in df.iterrows():
            ...
            insert_command = insert(summary).values(
...
def create_allprograms_table():
    ...
    with engine.connect() as conn:
        for index, row in df.iterrows():
            ...
            insert_command = insert(allprograms).values(
                state=val_state,
